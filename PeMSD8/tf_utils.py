...
def max_pool2d(x, kernel_size, stride = [2, 2], padding = 'VALID'):
    x = tf.nn.max_pool(
        x, [1] + kernel_size + [1], [1] + stride + [1], padding = padding)
    return x

def avg_pool2d(x, kernel_size, stride = [2, 2], padding = 'VALID'):
    x = tf.nn.avg_pool2d(
        x, [1] + kernel_size + [1], [1] + stride + [1], padding = padding)
    return x
...
def dropout(x, drop_rate, is_training):
    x = tf.cond(
        is_training,
